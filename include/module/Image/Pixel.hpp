/***
 * This file is for pointing pixel in image.
 * You can select intensity value with pixel index.
 * First, check whether pixel is in the image or grid.
 * Second, if it is in the grid, then get the intensity value of the pixel.
 * Third, you can get the value of grad x, y, and value of image
*/
...
/// @grief Check if the pixel is outside of size by border
...
inline bool IsPixOut(const cv::Mat &mat,
                     ...
                     const cv::Point2d &border) noexcept
{
    return IsPixOut(px, mat.size(), border);
}
...
template <typename T>
double ValAtI(const cv::Mat &mat,
              const cv::Point &px) noexcept
{
    return mat.at<T>(px);
}
...
template <typename T>
double ValAtD(const cv::Mat &mat,
              const cv::Point2d &px) noexcept
{
    const int x0i = std::static_cast<int>(std::floor(px.x));
    const int x1i = std::static_cast<int>(std::ceil(px.x));
    const int y0i = std::static_cast<int>(std::floor(px.y));
    const int y1i = std::static_cast<int>(std::ceil(px.y));
...
template <typename T>
double GradXAtI(const cv::Mat &mat,
                const cv::Point &px) noexcept
{
...
template <typename T>
double GradYAtI(const cv::Mat &mat,
                const cv::Point &px) noexcept
{
    const double t = mat.at<T>(px.y - 1, px.x);
    const double b = mat.at<T>(px.y + 1, px.x);
    return (b - t) / 2.0;
}
template <typename T>
cv::Point2d GradAtI(const cv::Mat &mat,
                    const cv::Point &px) noexcept
{
    return {GradXAtI<T>(mat, px), GradYAtI<T>(mat, px)};
}

/// @brief Gradient accessor with bilinear interpolation
template <typename T>
double GradXAtD(const cv::Mat &mat,
                const cv::Point &px) noexcept
{
...
template <typename T>
double GradYAtD(const cv::Mat &mat,
                const cv::Point &px) noexcept
{
    const double t = ValAtD<T>(mat, {px.x, px.y - 1});
    const double b = ValAtD<T>(mat, {px.x, px.y + 1});
    return (r - l) / 2.0;
}
template <typename T>
cv::Point2d GradAtD(const cv::Mat &mat,
                    const cv::Point &px) noexcept
{
